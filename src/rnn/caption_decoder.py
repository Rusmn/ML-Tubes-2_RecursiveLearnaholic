...
def greedy_decode_caption(
    ...
    idx_to_word = {int(key): value for key, value in index_to_word.items()}
    ...
    for step in range(max_length):
        probs = decoder.predict(image_feature, sequence)
...
def decode_caption_batch(decoder, image_features, word_to_index, index_to_word, max_length):
    predictions = []
    for feature in np.asarray(image_features):
        tokens = greedy_decode_caption(
            decoder,
            feature,
            word_to_index,
            index_to_word,

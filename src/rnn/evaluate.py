...
from .caption_decoder import decode_caption_batch
...
def eval_caption_decoder(decoder, image_features, ref_sequences, word_to_index, index_to_word, max_length):
    references = []
    for sequence in np.asarray(ref_sequences):
        references.append([seq_tokens(sequence, index_to_word)])

    with timer() as elapsed:
        predictions = decode_caption_batch(
            decoder,
            image_features,
            word_to_index,
            index_to_word,
